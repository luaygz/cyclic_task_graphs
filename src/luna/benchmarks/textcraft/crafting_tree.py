"""Recipe graph adapted from the MIT-licensed ADaPT TextCraft environment."""
...
# Luna imports
from luna.benchmarks.textcraft.utils import ItemTag, ItemTagWithCount, Recipe, ActionFailed, item_id_to_str
...
class CraftingTree:
    ...
    def traverse_recipe_tree(self, item_name: str, visited_names: Set[str]):
        if item_name in visited_names:
            ...
            return []
        # Copy before extending. The upstream implementation reused the stored
        # list and accidentally appended dependency recipes to the item itself.
    ...
    def item_recipes_min_depth(self, min_depth: int, max_depth: int):
        for item, recipes in self.itemid_recipes.items():
            item_depth = self.get_min_depth(item)
            if item_depth >= min_depth and item_depth <= max_depth:
    ...
    def create_recipe_set(self, item_name: str):
        ...
        # Fix: Ensure recipes for generic tag ingredients are included
        # Check both main recipes and distractors for tag requirements
        all_recipes = list(recipes) + distractors
        additional_recipes = self._add_missing_tag_recipes(all_recipes)
    ...
    def _add_missing_tag_recipes(self, recipes: List[Recipe]) -> List[Recipe]:
        """
        Add recipes for specific items that satisfy generic tag requirements.
        
        For example, if a recipe requires "wooden slabs" but we don't have recipes 
        to create specific slabs like "oak slab", this method will add those.
        
        Args:
            recipes: List of existing recipes
            
        Returns:
            List of additional recipes needed to satisfy tag dependencies
        """
        additional_recipes = []
        required_tags = set()
        
        # Collect all items that are already specifically mentioned in recipes
        mentioned_items = set()
        for recipe in recipes:
            # Add the output item
            if recipe.output_item.item_tag.item_id:
                mentioned_items.add(recipe.output_item.item_tag.item_id)
            
            # Add input items
            for input_item in recipe.input_items:
                if input_item.item_tag.item_id:
                    mentioned_items.add(input_item.item_tag.item_id)
                    
                    # Check if this item_id should be mapped to a tag
                    # e.g., "minecraft:wooden_slabs" -> "minecraft:wooden_slab" tag
                    potential_tag = self._item_id_to_tag_mapping(input_item.item_tag.item_id)
                    if potential_tag and potential_tag in self.tag_set:
                        required_tags.add(potential_tag)
                        
                elif input_item.item_tag.tag:
                    required_tags.add(input_item.item_tag.tag)
        
        # For each required tag, add recipes for specific items that:
        # 1. Have that tag
        # 2. Are mentioned elsewhere in the recipe set (or their variants are)
        for tag in required_tags:
            if tag in self.tag_set:
                # Get all specific items that have this tag
                tagged_items = list(self.get_items_with_tags(tag))
                
                # Find which of these items should be included based on what's mentioned
                items_to_include = []
                for item in tagged_items:
                    # Check if this specific item or any related items are mentioned
                    if item in mentioned_items:
                        items_to_include.append(item)
                    else:
                        # Check if any items from the same "family" are mentioned
                        # For example, if "oak_planks" is mentioned, include "oak_slab"
                        item_base = self._get_item_base(item)
                        for mentioned_item in mentioned_items:
                            mentioned_base = self._get_item_base(mentioned_item)
                            if item_base == mentioned_base and item_base != "minecraft":
                                items_to_include.append(item)
                                break
                
                # If no specific items found, include at least a few common ones
                if not items_to_include and tagged_items:
                    # Prefer common wood types
                    common_items = [item for item in tagged_items 
                                   if any(wood in item for wood in ["oak", "birch", "spruce"])]
                    items_to_include = common_items[:3] if common_items else tagged_items[:3]
                
                # Add recipes for the selected items
                for item in items_to_include:
                    if item in self.itemid_recipes:
                        item_recipes = self.itemid_recipes[item]
                        # Add recipes that aren't already in the main recipe set
                        for recipe in item_recipes:
                            if recipe not in recipes and recipe not in additional_recipes:
    ...
    def _get_item_base(self, item: str) -> str:
        """
        Get the base material type from an item name.
        
        For example:
        - "minecraft:oak_planks" -> "minecraft:oak"
        - "minecraft:oak_slab" -> "minecraft:oak"  
        - "minecraft:birch_stairs" -> "minecraft:birch"
        
        Args:
            item: Full item name like "minecraft:oak_planks"
            
        Returns:
            Base material name like "minecraft:oak"
        """
        if not item.startswith("minecraft:"):
            return item
            
        # Remove minecraft: prefix
        item_without_prefix = item[10:]
        
        # Common item suffixes to remove
        suffixes = ["_planks", "_slab", "_stairs", "_fence", "_door", "_trapdoor", 
                   "_button", "_pressure_plate", "_sign", "_boat", "_logs", "_wood",
                   "_fence_gate", "_hyphae", "_stems"]
        
        for suffix in suffixes:
            if item_without_prefix.endswith(suffix):
                return "minecraft:" + item_without_prefix[:-len(suffix)]
        
        return item

    ####################################################################################################

    def _item_id_to_tag_mapping(self, item_id: str) -> str:
        """
        Map item IDs that don't exist as craftable items to their corresponding tags.
        ...
        Returns:
            Corresponding tag name, or None if no mapping exists
        """
        # Common mappings from plural/generic item IDs to tags
        mappings = {
            "minecraft:wooden_slabs": "minecraft:wooden_slab",
            "minecraft:planks": "minecraft:planks",
            "minecraft:logs": "minecraft:logs",
            "minecraft:stones": "minecraft:stone",
            "minecraft:wools": "minecraft:wool",
        }
        
        return mappings.get(item_id)
